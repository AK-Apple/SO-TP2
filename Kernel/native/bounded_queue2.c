...
uint64_t enqueue_string2(queue2_t* q, char* string, uint64_t size) {

    if (MAX_SIZE - q->size < size) {
        size = MAX_SIZE - q->size;
    }

    // Calculate the available space in the queue
    int available_space = MAX_SIZE - q->rear;
...
uint64_t dequeue_to_buffer2(queue2_t* q, char* buffer, uint64_t size) {
    ...
    uint64_t to_return_size = q->size;

    // Determine how much can be copied in one go
    int available_space = MAX_SIZE - q->front;
    ...
    q->front = (q->front + to_return_size) % MAX_SIZE;
    q->size = 0; // Set the size to zero since the queue is now empty
